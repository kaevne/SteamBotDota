...
namespace SteamTrade
{
    ...
    public class ForeignInventory
    {
        ...
        /// <summary>
        /// Gets the rarity for a given Item ID.
        /// </summary>
        /// <param name="itemId">The item id.</param>
        /// <returns>A rarity of -1 if there is an error.</returns>
        public string GetRarity(ulong itemId)
        {
            uint classId = GetClassIdForItemId(itemId);
            ulong iid = GetInstanceIdForItemId(itemId);
            ...
            try
            {
                ...
                // descriptions object. this may not be the case for all
                // games and therefore this may be non-portable.
                r = rawJson.rgDescriptions[index].tags[1].name;
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        public string GetTrueName(ulong itemId)
        {
            uint classId = GetClassIdForItemId(itemId);
            ulong iid = GetInstanceIdForItemId(itemId);
            ...
            try
            {
                ...
                // descriptions object. this may not be the case for all
                // games and therefore this may be non-portable.
                r = rawJson.rgDescriptions[index].tags[0].name;
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
